...
lwe=pd.read_csv("data/LWE_2013_TRANSPORT.csv", sep=";", encoding = "utf-8")
...
# main code
survey_dict={}
nb_reconstructed_tours=0
nb_simplified_tours=0
nb_legs=0
nb_shipments=0
...
for i in range (0, nb_rows):
    if i == 0:
        survey_inst=LWE_classes.Survey(oid[i], wh_tot_cal[i], strata[i],
                                       ...
                                       bool(main_use_service[i]), curb_weight[i], load_capa[i])
        ...
        nb_reconstructed_tours += len(survey_inst.reconstructed_tour_list)
        nb_simplified_tours += len(survey_inst.simplified_tour_list)
        nb_legs += len(survey_inst.leg_list)
        nb_shipments += len(survey_inst.shipment_list)
...
shipment_data=np.empty((nb_shipments, 11))
k = 0
...
header = ['oid', 'weight', 'wg', 'load', 'length', 'NPA_O', 'NPA_D', 'IsFirstLeg', 'IsLastLeg', 'km_o', 'km_d']
