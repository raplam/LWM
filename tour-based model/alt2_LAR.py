...
def alt2_folder_LAR(input_folder, nb_stops_by_trips, beta_dem, beta_ini, beta_inter, epsilon, max_steps):
    ...
    output_file_name = output_folder + "/" +output_folder[output_folder.rfind("/")+1:] + \
                       "_alt2_nb" + str(nb_stops_by_trips) + \
    ...
    alt2_LAR(prod_file, attr_file, costs_file, nb_stops_by_trips, beta_dem, beta_ini, beta_inter, epsilon, max_steps, output_file_name, input_folder[input_folder.rfind("/")+1:]+"_LAR")

def alt2_LAR(prod_file, attr_file, costs_file, nb_stops_by_trips, beta_dem, beta_ini, beta_inter, epsilon, max_steps, output_file_name, folder_name):
    ...
    for o in range(0,N-1):
      ...
      intermediate_trips_from_o=np.zeros((1,N))
      current_positions=initial_trips[o,:]
      for k in range(1,nb_stops_by_trips):
        # Furness for legs of rank k
        intermediate_trips_rank_k_from_o=tools.Furness_LAR(np.exp(-beta_inter*savings),demand[o,:]-np.sum(intermediate_trips_from_o+initial_trips[o,:],axis=0),current_positions, True, False, epsilon, max_steps)
        # add them to the previous ones
        intermediate_trips_from_o = intermediate_trips_from_o + intermediate_trips_rank_k_from_o
        # update the current positions
        current_positions=np.sum(intermediate_trips_rank_k_from_o,axis=0)
